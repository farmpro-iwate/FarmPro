...
export function StaffSearchField({ value, masterId, onChange, label = '獣医師名' }: Props) {
  ...
  useEffect(() => {
    async function loadStaff() {
      ...
      try {
        ...
        setStaff(masters.filter((master) => master.active).map((master) => ({ id: master.id, name: master.name, code: master.code, note: master.note })));
      } catch (err) {
  ...
  const selected = staff.find((item) => item.id === masterId) || staff.find((item) => item.name === value) || null;
  ...
  async function handleCreate() {
    ...
    setError('');
    try {
      const created = await createMaster({ category: 'veterinarian', name, code: newCode.trim() || undefined, note: newNote.trim() || undefined });
      const option: StaffOption = { id: created.id, name: created.name, code: created.code, note: created.note };
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'stretch', sm: 'flex-start' }}>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Autocomplete
            ...
            onClose={(_, reason) => {
              if (reason === 'blur' && isUnregisteredMasterName(value, staff)) openCreateDialog();
            }}
            ...
            freeSolo
            renderInput={(params) => <TextField {...params} label={label} placeholder="名前・コード・備考で検索..." fullWidth />}
      ...
      <Dialog open={openDialog} onClose={closeDialog} fullWidth maxWidth="sm" PaperProps={{ sx: { m: 1, width: 'calc(100% - 16px)' } }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2}>
            ...
            <TextField label="コード（任意）" value={newCode} onChange={(event) => setNewCode(event.target.value)} placeholder="例：VET-01 / AI-01" fullWidth disabled={creating} />
            <TextField label="備考（任意）" value={newNote} onChange={(event) => setNewNote(event.target.value)} placeholder="例：所属先、担当分野など" multiline minRows={2} fullWidth disabled={creating} />
