...
export function FeedSearchField({ value, onChange, required = false }: Props) {
      ...
      try {
        ...
        setFeeds(
          masters
            ...
            .map((master) => ({ id: master.id, name: master.name, code: master.code, note: master.note }))
        );
      } catch (err) {
  ...
  async function handleCreate() {
    ...
    setError('');
    try {
      const created = await createMaster({ category: 'feed', name, code: newFeedCode.trim() || undefined, note: newFeedNote.trim() || undefined });
      const option: FeedOption = { id: created.id, name: created.name, code: created.code, note: created.note };
    ...
    <Stack spacing={1}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'stretch', sm: 'flex-start' }}>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Autocomplete
            ...
            onClose={(_, reason) => {
              if (reason === 'blur' && isUnregisteredMasterName(value, feeds)) openCreateDialog();
            }}
            ...
            freeSolo
            renderInput={(params) => <TextField {...params} label="飼料名" placeholder="飼料名またはコードで検索..." required={required} fullWidth />}
      ...
      {selectedFeed && value && (
        <Box sx={{ p: 1.5, bgcolor: '#f5f5f5', border: '2px solid #4caf50', borderRadius: 1, minWidth: 0, wordBreak: 'break-word' }}>
          <Typography fontWeight={800} sx={{ fontSize: '1.1rem', color: '#1976d2' }}>✓ {selectedFeed.name}</Typography>
          {selectedFeed.code && <Typography sx={{ mt: 0.5 }}>コード：{selectedFeed.code}</Typography>}
          {selectedFeed.note && <Typography color="text.secondary" sx={{ mt: 0.5 }}>{selectedFeed.note}</Typography>}
        </Box>
      )}
      ...
      <Dialog open={openDialog} onClose={closeDialog} fullWidth maxWidth="sm" PaperProps={{ sx: { m: 1, width: 'calc(100% - 16px)' } }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2}>
            ...
            <TextField label="飼料名 *" value={newFeedName} onChange={(event) => setNewFeedName(event.target.value)} placeholder="例：配合飼料、乾草、稲わら" autoFocus fullWidth disabled={creating} />
            <TextField label="コード（任意）" value={newFeedCode} onChange={(event) => setNewFeedCode(event.target.value)} placeholder="例：HAIGO-01" fullWidth disabled={creating} />
            <TextField label="備考（任意）" value={newFeedNote} onChange={(event) => setNewFeedNote(event.target.value)} placeholder="例：育成用、購入先など" multiline minRows={2} fullWidth disabled={creating} />
            {error && <Alert severity="error">{error}</Alert>}
          </Stack>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button type="button" onClick={closeDialog} disabled={creating}>キャンセル</Button>
          <Button type="button" variant="contained" onClick={handleCreate} disabled={creating || !newFeedName.trim()}>{creating ? <CircularProgress size={20} /> : '登録'}</Button>
        </DialogActions>
      </Dialog>
    </Stack>
  );
