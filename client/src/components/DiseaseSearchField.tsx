...
export function DiseaseSearchField({ value, masterId, onChange, label = '疾病名', required = false }: Props) {
  ...
  useEffect(() => {
    async function loadDiseases() {
      ...
      try {
        ...
        setDiseases(masters.filter((master) => master.active).map((master) => ({ id: master.id, name: master.name, code: master.code, note: master.note })));
      } catch (err) {
  ...
  const selected = diseases.find((item) => item.id === masterId) || diseases.find((item) => item.name === value) || null;
  ...
  async function handleCreate() {
    ...
    setError('');
    try {
      const created = await createMaster({ category: 'disease', name, code: newCode.trim() || undefined, note: newNote.trim() || undefined });
      const option: DiseaseOption = { id: created.id, name: created.name, code: created.code, note: created.note };
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'stretch', sm: 'flex-start' }}>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Autocomplete
            ...
            onClose={(_, reason) => {
              if (reason === 'blur' && isUnregisteredMasterName(value, diseases)) openCreateDialog();
            }}
            ...
            freeSolo
            renderInput={(params) => <TextField {...params} label={label} placeholder="疾病名またはコードで検索..." required={required} fullWidth />}
      ...
      <Dialog open={openDialog} onClose={closeDialog} fullWidth maxWidth="sm" PaperProps={{ sx: { m: 1, width: 'calc(100% - 16px)' } }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2}>
            ...
            <TextField label="疾病名 *" value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="例：乳房炎、肺炎、蹄病" autoFocus fullWidth disabled={creating} />
            <TextField label="コード（任意）" value={newCode} onChange={(event) => setNewCode(event.target.value)} placeholder="例：DIS-001" fullWidth disabled={creating} />
            <TextField label="備考（任意）" value={newNote} onChange={(event) => setNewNote(event.target.value)} placeholder="例：慢性化しやすい、再発注意など" multiline minRows={2} fullWidth disabled={creating} />
