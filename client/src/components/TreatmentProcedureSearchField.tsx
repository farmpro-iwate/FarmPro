...
export function TreatmentProcedureSearchField({ value, masterId, onChange, required = false }: Props) {
  ...
  useEffect(() => {
    async function loadProcedures() {
      ...
      try {
        ...
        setProcedures(masters.filter((master) => master.active).map((master) => ({ id: master.id, name: master.name, code: master.code, note: master.note })));
      } catch (err) {
  ...
  const selected = procedures.find((item) => item.id === masterId) || procedures.find((item) => item.name === value) || null;
  ...
  async function handleCreate() {
    ...
    setError('');
    try {
      const created = await createMaster({ category: 'treatmentProcedure', name, code: newCode.trim() || undefined, note: newNote.trim() || undefined });
      const option: ProcedureOption = { id: created.id, name: created.name, code: created.code, note: created.note };
    ...
    <Stack spacing={1}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} alignItems={{ xs: 'stretch', sm: 'flex-start' }}>
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Autocomplete
            ...
            onClose={(_, reason) => {
              if (reason === 'blur' && isUnregisteredMasterName(value, procedures)) openCreateDialog();
            }}
            ...
            freeSolo
            renderInput={(params) => <TextField {...params} label="処置内容" placeholder="処置内容またはコードで検索..." required={required} fullWidth />}
      ...
      {selected && value && (
        <Box sx={{ p: 1.5, bgcolor: '#f5f5f5', border: '2px solid #4caf50', borderRadius: 1, minWidth: 0, wordBreak: 'break-word' }}>
          <Typography fontWeight={800} sx={{ fontSize: '1.1rem', color: '#1976d2' }}>✓ {selected.name}</Typography>
      ...
      <Dialog open={openDialog} onClose={closeDialog} fullWidth maxWidth="sm" PaperProps={{ sx: { m: 1, width: 'calc(100% - 16px)' } }}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Stack spacing={2}>
            ...
            <TextField label="処置内容 *" value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="例：点滴、洗浄、縫合、消炎処置" autoFocus fullWidth disabled={creating} />
            <TextField label="コード（任意）" value={newCode} onChange={(event) => setNewCode(event.target.value)} placeholder="例：PROC-001" fullWidth disabled={creating} />
            <TextField label="備考（任意）" value={newNote} onChange={(event) => setNewNote(event.target.value)} placeholder="例：処置手順メモ、再処置の目安など" multiline minRows={2} fullWidth disabled={creating} />
            {error && <Alert severity="error">{error}</Alert>}
          </Stack>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button type="button" onClick={closeDialog} disabled={creating}>キャンセル</Button>
          <Button type="button" variant="contained" onClick={handleCreate} disabled={creating || !newName.trim()}>{creating ? <CircularProgress size={20} /> : '登録'}</Button>
        </DialogActions>
      </Dialog>
    </Stack>
  );
