...
export function CattleForm({ mode }: Props) {
  ...
  const handleSubmit = async () => { if (!form.earTag || !form.name || !form.birthday) { alert('個体番号、名号、生年月日は必須です'); return; } try { if (mode === 'create') await createCattle(form); else if (id) await updateCattle(id, form); navigate('/cattle'); } catch { alert('保存に失敗しました。'); } };
  if (loading) return <Typography>読み込み中...</Typography>;
  return (
    <Stack spacing={2}><Typography variant="h5" fontWeight={800}>{mode === 'create' ? '牛を新規登録' : '牛を編集'}</Typography>
      <Card><CardContent><Stack spacing={2}>
        ...
        <TextField label="BLV結果" select value={form.blvStatus} onChange={(e) => setValue('blvStatus', e.target.value)} fullWidth><MenuItem value="未検査">未検査</MenuItem><MenuItem value="陰性">陰性</MenuItem><MenuItem value="陽性">陽性</MenuItem></TextField>
        <TextField label="備考" value={form.note} onChange={(e) => setValue('note', e.target.value)} multiline minRows={3} fullWidth />
        <Stack direction="row" spacing={1}><Button variant="contained" size="large" onClick={handleSubmit}>保存</Button><Button component={RouterLink} to="/cattle" variant="outlined" size="large">戻る</Button></Stack>
