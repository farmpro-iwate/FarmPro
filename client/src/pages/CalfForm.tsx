...
const initialForm: CalfInput = { calfNumber: '', name: '', birthday: '', sex: '雌', motherName: '', startWeight: 0, currentWeight: 0, elapsedDays: 0, milkAmount: 0, starterAmount: 0, note: '' };

export function CalfForm({ mode }: Props) {
  const navigate = useNavigate(); const { id } = useParams();
  const [form, setForm] = useState<CalfInput>(initialForm); const [loading, setLoading] = useState(mode === 'edit');
  useEffect(() => { if (mode === 'edit' && id) getCalf(id).then((d) => setForm({ calfNumber: d.calfNumber, name: d.name, birthday: d.birthday, sex: d.sex, motherName: d.motherName, startWeight: d.startWeight, currentWeight: d.currentWeight, elapsedDays: d.elapsedDays, milkAmount: d.milkAmount, starterAmount: d.starterAmount, note: d.note })).finally(() => setLoading(false)); }, [mode, id]);
  const setValue = (key: keyof CalfInput, value: string | number) => setForm((prev) => ({ ...prev, [key]: value }));
  const handleSubmit = async () => { if (!form.calfNumber || !form.name || !form.birthday) { alert('子牛番号、名号、生年月日は必須です'); return; } try { if (mode === 'create') await createCalf(form); else if (id) await updateCalf(id, form); navigate('/calves'); } catch { alert('保存に失敗しました。'); } };
  if (loading) return <Typography>読み込み中...</Typography>;
  const dg = calculateDg(form.startWeight, form.currentWeight, form.elapsedDays);
  return <Stack spacing={2}><Typography variant="h5" fontWeight={800}>{mode === 'create' ? '子牛を新規登録' : '子牛を編集'}</Typography><Card><CardContent><Stack spacing={2}>
    ...
    <TextField label="性別" select value={form.sex} onChange={(e) => setValue('sex', e.target.value)} fullWidth><MenuItem value="雌">雌</MenuItem><MenuItem value="雄">雄</MenuItem></TextField>
    ...
    <Stack direction="row" spacing={1}><Button variant="contained" size="large" onClick={handleSubmit}>保存</Button><Button component={RouterLink} to="/calves" variant="outlined" size="large">戻る</Button></Stack>
