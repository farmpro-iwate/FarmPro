...
export function AnimalImportPage() {
  ...
  const registrationReady = Boolean(candidate && registrationEarTag.trim() && candidate.name.trim() && candidate.birthday.trim());
    ...
    <Stack spacing={2}>
      ...
      {candidate && <Card><CardContent><Stack spacing={2}>
        ...
        {showRegistrationReview && !hasStrongDuplicate && <Card variant="outlined"><CardContent><Stack spacing={1.5}>
          ...
          <TextField label="性別" select value={registrationSex} onChange={(e) => setRegistrationSex(e.target.value as '雌' | '雄' | '去勢')} required>
          ...
          {registrationReady ? <Alert severity="success">基本登録に必要な項目が揃っています。</Alert> : <Alert severity="warning">耳標番号・名号・生年月日を確認してください。</Alert>}
