...
function normalizeEraDate(raw: string) {
  ...
  if (westernMatch) {
    return `${westernMatch[1]}-${String(Number(westernMatch[2])).padStart(2, '0')}-${String(Number(westernMatch[3])).padStart(2, '0')}`;
  }
...
function parseOcrCandidate(text: string): CattleImportCandidate {
  ...
  const identificationNumber =
    text.match(/\b\d{4}[-－]\d{4}[-－]\d\b/)?.[0]?.replace(/－/g, '-') || '';
  const registrationNumber =
    text.match(/\b\d{3}[-－]\d{7}\b/)?.[0]?.replace(/－/g, '-') || '';
  ...
  for (const line of lines) {
    ...
    if (rowBirthday && rowName && rowName !== name) {
      offspring.push({ parity, name: rowName, birthday: rowBirthday, sire: rowSire });
    }
  }
...
async function pdfFileToCanvas(file: File) {
  const pdfjs = await import(
    /* @vite-ignore */ 'https://cdn.jsdelivr.net/npm/pdfjs-dist@6.2.108/build/pdf.mjs'
  );
...
export function AnimalImportPage() {
  ...
  const handleReadDocument = async () => {
    ...
    setOcrText('');
    setOcrProgress(0);
    setOcrStatus('帳票を画像へ変換しています…');
    setOcrRunning(true);

    let worker: any = null;
    try {
      const canvas = documentPreview.isImage
        ? await imageFileToCanvas(documentFile)
        : await pdfFileToCanvas(documentFile);

      setOcrStatus('日本語OCRを準備しています…');
      const tesseract = await import(
        /* @vite-ignore */ 'https://cdn.jsdelivr.net/npm/tesseract.js@7/dist/tesseract.esm.min.js'
      );
      ...
      setOcrText(text);
      setCandidate(parseOcrCandidate(text));
      setOcrProgress(100);
      setOcrStatus('読み取り候補を作成しました。');
    } catch (caught) {
      setError(
  ...
  const updateCandidate = (key: keyof Omit<CattleImportCandidate, 'offspring'>, value: string) => {
    setCandidate((current) => ({ ...(current || emptyCandidate), [key]: value }));
  };
        ...
        <CardContent>
          <Stack spacing={2}>
            ...
            <Typography color="text.secondary">
              牛の通信簿、成績表、JA・市場などの類似帳票を選択します。帳票名ではなく、個体情報・血統・産歴・子牛・販売成績などの項目として読み取ります。
            </Typography>

            <Button component="label" variant="contained" size="large" disabled={ocrRunning}>
              画像・PDFを選ぶ
              <input
                hidden
                type="file"
              ...
              <Card variant="outlined">
                <CardContent>
                  <Stack spacing={1.5}>
                    ...
                    {!documentPreview.isImage && (
                      <Alert severity="info">
                        PDFは1ページ目を画像化して読み取ります。複数ページ対応は後続工程で追加します。
                      </Alert>
                    )}
                    <Alert severity="warning">
                      読み取り結果は候補です。確認しても、この段階ではまだ正式データへ保存しません。
                    </Alert>
                    <Button variant="contained" onClick={handleReadDocument} disabled={ocrRunning}>
                      {ocrRunning ? '読み取り中…' : 'この帳票を読み取る'}
                    </Button>
                    {ocrRunning && (
                      <Stack spacing={0.5}>
                    ...
                    <Button variant="outlined" onClick={resetDocumentPreview} disabled={ocrRunning}>
                      選び直す
                    </Button>
                  </Stack>
                </CardContent>
              ...
              <Card variant="outlined">
                <CardContent>
                  <Stack spacing={2}>
                    ...
                    <Alert severity="info">
                      OCRの候補をFarmPro標準項目へ当てはめています。誤読した欄はここで修正できます。まだ保存されません。
                    </Alert>
                    ...
                    {candidate.offspring.length === 0 ? (
                      <Alert severity="warning">産歴の表はまだ自動で判定できませんでした。次工程で表の読み取り精度を調整します。</Alert>
                    ) : (
                      <Stack spacing={1}>
                        {candidate.offspring.map((row, index) => (
                          <Card key={`${row.parity}-${index}`} variant="outlined">
                            <CardContent>
                              <Typography fontWeight={800}>{row.parity}産　{row.name || '名号未判定'}</Typography>
                              <Typography color="text.secondary">生年月日：{row.birthday || '-'}　父牛：{row.sire || '-'}</Typography>
                            </CardContent>
                          </Card>
                        ))}
                      </Stack>
                    ...
                    <Divider />
                    <Typography fontWeight={900}>OCR原文（確認用）</Typography>
                    <TextField value={ocrText} multiline minRows={8} fullWidth InputProps={{ readOnly: true }} />
                    <Alert severity="warning">
                      「一括登録」はまだ接続していません。次工程で、重複確認と登録前確認を追加してから正式保存できるようにします。
                    </Alert>
                  </Stack>
                </CardContent>
              </Card>
