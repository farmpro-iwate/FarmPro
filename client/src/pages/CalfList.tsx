...
export function CalfList() {
  ...
  const loadCalves = async () => {
    setLoading(true);
    setCalves(await getCalfList());
    setLoading(false);
  };
  ...
  const handleDelete = async (calf: Calf) => {
    if (!window.confirm(`${calf.name}（${calf.calfNumber}）を削除しますか？`)) return;
    await deleteCalf(calf.id);
    await loadCalves();
  };
    ...
    <Stack spacing={2}>
      ...
      <Card><CardContent>
        {loading ? <Typography>読み込み中...</Typography> : (
          <Table size="small">
            <TableHead><TableRow>
              <TableCell>子牛番号</TableCell><TableCell>名号</TableCell><TableCell>日齢</TableCell><TableCell>DG</TableCell><TableCell>判定</TableCell><TableCell align="right">操作</TableCell>
            </TableRow></TableHead>
            <TableBody>
              {calves.map((calf) => {
                const dg = calculateDg(calf.startWeight, calf.currentWeight, calf.elapsedDays);
                return (
                  <TableRow key={calf.id}>
                    <TableCell>{calf.calfNumber}</TableCell>
                    <TableCell>{calf.name}</TableCell>
                    <TableCell>{calculateAgeDays(calf.birthday)}日</TableCell>
                    ...
                    <TableCell align="right">
                      <IconButton component={RouterLink} to={`/calves/${calf.id}/edit`} aria-label="編集"><EditIcon /></IconButton>
                      <IconButton color="error" onClick={() => handleDelete(calf)} aria-label="削除"><DeleteIcon /></IconButton>
                    </TableCell>
                  </TableRow>
                );
              })}
