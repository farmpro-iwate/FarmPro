...
function downloadCsv(rows: SaleRecord[]) {
  const headers = [
    '状態', '区分', '対象番号', '対象名', '性別', '生年月日', '母牛', '出荷予定日', '出荷日', '販売日',
    '販売先・購買者', '市場名', '販売体重', '販売金額', '販売理由', 'メモ', '作成日時', '更新日時'
  ];
  const body = rows.map((row) => [
    row.status, row.targetType, row.targetNumber, row.targetName, row.sex, row.birthday, row.motherName,
    row.shippingPlanDate, row.shippingDate, row.saleDate, row.buyer, row.marketName, row.saleWeight, row.salePrice,
    row.reason, row.memo, row.createdAt, row.updatedAt
  ]);
...
export function SalesList() {
  ...
  async function loadSales() {
    setLoading(true);
    try { setRows(await getSalesList()); setError(''); }
  ...
  async function handleDelete(row: SaleRecord) {
    ...
    if (!window.confirm(`${label} の出荷・販売記録を削除しますか？\n削除すると元に戻せません。`)) return;
    setDeletingId(row.id); setError('');
    try { await deleteSale(row.id); setRows((prev) => prev.filter((item) => item.id !== row.id)); }
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (!q) return true;
      return [row.targetType, row.targetNumber, row.targetName, row.sex, row.motherName, row.shippingPlanDate,
        row.shippingDate, row.saleDate, row.buyer, row.marketName, row.saleWeight, row.salePrice, row.status,
        row.reason, row.memo].join(' ').toLowerCase().includes(q);
    });
  }, [rows, keyword, statusFilter, targetTypeFilter]);

  const totalPrice = useMemo(() => filteredRows.reduce((sum, row) => {
    const n = Number(row.salePrice); return Number.isNaN(n) ? sum : sum + n;
  }, 0), [filteredRows]);
      ...
      {!loading && !error && filteredRows.length > 0 && <>
        <Stack spacing={1.5} sx={{ display: { xs: 'flex', md: 'none' } }} className="no-print">
          {filteredRows.map((row) => <Card key={row.id}><CardContent><Stack spacing={1.25}>
            <Stack direction="row" spacing={1} justifyContent="space-between" alignItems="flex-start">
              <Box><Typography variant="h6" fontWeight={800}>{value(row.targetName)}</Typography><Typography color="text.secondary">{value(row.targetType)} / {value(row.targetNumber)}</Typography></Box>
