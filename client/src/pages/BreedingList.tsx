...
export function BreedingList() {
  ...
  const filteredItems = useMemo(() => {
    const filtered = items.filter((item) =>
      matchesAnyText([item.cowEarTag, item.cowName, item.breedingMethod, item.breedingStatus, item.pregnancyResult, breedingSire(item), breedingActor(item), breedingPartner(item), item.note], keyword) &&
  ...
  const filterActive = Boolean(keyword.trim() || method !== 'すべて' || stage !== 'すべて' || result !== 'すべて');
          ...
          {loading ? <Typography>読み込み中...</Typography> : filteredItems.length === 0 ? <Typography color="text.secondary">条件に合う繁殖記録はありません。</Typography> : isMobile ? (
            ...
            <Box sx={{ overflowX: 'auto' }}>
              <Table size="small" sx={{ minWidth: 1120 }}>
                <TableHead><TableRow><TableCell>耳標番号・牛名</TableCell><TableCell>繁殖方法</TableCell><TableCell>実施日</TableCell><TableCell>父牛・担当者</TableCell><TableCell>現在の段階</TableCell><TableCell>次対応・予定日</TableCell><TableCell>分娩予定</TableCell><TableCell>受胎確認</TableCell><TableCell>注意</TableCell><TableCell align="right">操作</TableCell></TableRow></TableHead>
                <TableBody>{filteredItems.map((item) => <TableRow key={item.id} hover>
                  ...
                  <TableCell align="right"><Stack direction="row" justifyContent="flex-end" spacing={0.5}>{currentStage(item) === '分娩待ち' && <Button component={RouterLink} to={calvingRegistrationUrl(item)} size="small" variant="contained">分娩登録</Button>}<IconButton component={RouterLink} to={`/breedings/${item.id}/edit`} aria-label="編集"><EditIcon /></IconButton><IconButton color="error" onClick={() => handleDelete(item)} aria-label="削除"><DeleteIcon /></IconButton></Stack></TableCell>
                </TableRow>)}</TableBody>
              </Table>
            </Box>
          )}
        ...
        <Stack spacing={1}>
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
            ...
            <TextField label="現在の段階" select value={stage} onChange={(e) => setStage(e.target.value)} size="small" sx={{ minWidth: 170 }}>
              <MenuItem value="すべて">すべて</MenuItem><MenuItem value="発情予定">発情予定</MenuItem><MenuItem value="種付実施">種付実施</MenuItem><MenuItem value="妊娠鑑定待ち">妊娠鑑定待ち</MenuItem><MenuItem value="受胎確認">受胎確認</MenuItem><MenuItem value="分娩待ち">分娩待ち</MenuItem><MenuItem value="分娩済み">分娩済み</MenuItem><MenuItem value="経過観察">経過観察</MenuItem><MenuItem value="完了">完了</MenuItem>
            </TextField>
          ...
          <Button variant="outlined" onClick={clearSearch} size="small" disabled={!filterActive}>検索・絞り込みをクリア</Button>
        </Stack>
