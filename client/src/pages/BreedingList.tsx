...
function displayDate(value: string) { return value || '未登録'; }
function performedDate(item: Breeding) { return item.breedingMethod === '受精卵移植' ? item.transferDate : item.inseminationDate; }
...
function breedingActor(item: Breeding) { return item.breedingMethod === '受精卵移植' ? (item.transferTechnician || '未登録') : (item.inseminatorName || '未登録'); }
function breedingSire(item: Breeding) { return item.breedingMethod === '受精卵移植' ? (item.embryoSireName || '未登録') : (item.bullName || '未登録'); }
function breedingPartner(item: Breeding) { if (item.breedingMethod !== '受精卵移植') return '-'; return item.supplierName || '未登録'; }
...
function cautionMessages(item: Breeding) {
  ...
  if (checkDate && !item.pregnancyCheckDate && dateDiffDays(today, checkDate) < 0) messages.push('妊娠鑑定予定日を過ぎています');
  ...
  if (!item.nextHeatExpectedDate && !item.pregnancyCheckExpectedDate && !item.expectedCalvingDate) messages.push('次回予定日が不足しています');
  return messages;
}
...
function priorityRank(item: Breeding) {
  ...
  const today = new Date();
  const actionDate = parseDate(nextAction(item).date);
  if (actionDate) {
  ...
  if (item.pregnancyResult === '受胎' && item.expectedCalvingDate) {
    ...
    if (calving && dateDiffDays(today, calving) <= 14) return 3;
  }
  return 9;
}
  ...
  return (
    <Card variant="outlined"><CardContent><Stack spacing={1.5}>
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={1}>
        <Box><Typography variant="h6" fontWeight={800}>耳標：{item.cowEarTag || '未登録'}</Typography><Typography color="text.secondary">牛名：{item.cowName || '未登録'}</Typography></Box>
      ...
      <Stack spacing={0.75}>
        ...
        {item.breedingStatus === '中止' && item.transferCancelReason && <Typography color="error"><strong>中止理由：</strong>{item.transferCancelReason}</Typography>}
      ...
      <Stack direction="row" spacing={1}>
        ...
        <Button color="error" variant="outlined" startIcon={<DeleteIcon />} onClick={() => onDelete(item)} fullWidth>削除</Button>
      </Stack>
...
export function BreedingList() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      const aDate = parseDate(nextAction(a).date); const bDate = parseDate(nextAction(b).date);
  ...
  const handleDelete = async (item: Breeding) => { if (!window.confirm(`${item.cowName}の繁殖記録を削除しますか？`)) return; await deleteBreeding(item.id); await load(); };
  const clearSearch = () => { setKeyword(''); setMethod('すべて'); setStage('すべて'); setResult('すべて'); };
  const hasFilters = Boolean(keyword || method !== 'すべて' || stage !== 'すべて' || result !== 'すべて');

  return (
    <Stack spacing={1.5}>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} spacing={1}>
        <Stack spacing={0.25}><Typography variant="h5" fontWeight={800}>繁殖管理</Typography><Typography color="text.secondary">表示：{filteredItems.length}件 / 全{items.length}件</Typography></Stack>
      ...
      {searchOpen && (
        <Card><CardContent sx={{ py: 1.5 }}><Stack spacing={1}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
            <TextField label="検索" placeholder="耳標・牛名・方法・段階・父牛・担当者" value={keyword} onChange={(e) => setKeyword(e.target.value)} fullWidth size="small" />
            <TextField label="繁殖方法" select value={method} onChange={(e) => setMethod(e.target.value)} size="small" sx={{ minWidth: 150 }}>
              <MenuItem value="すべて">すべて</MenuItem><MenuItem value="未選択">未選択</MenuItem><MenuItem value="種付">種付</MenuItem><MenuItem value="受精卵移植">受精卵移植</MenuItem>
            </TextField>
            <TextField label="現在の段階" select value={stage} onChange={(e) => setStage(e.target.value)} size="small" sx={{ minWidth: 170 }}>
              {['すべて','発情予定','種付実施','妊娠鑑定待ち','受胎確認','分娩待ち','分娩済み','経過観察','完了'].map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}
            </TextField>
            <TextField label="受胎確認" select value={result} onChange={(e) => setResult(e.target.value)} size="small" sx={{ minWidth: 170 }}>
              {['すべて','未鑑定','再鑑定予定','受胎','空胎','流産・胎子喪失'].map((value) => <MenuItem key={value} value={value}>{value}</MenuItem>)}
            </TextField>
        ...
        {loading ? <Typography>読み込み中...</Typography> : filteredItems.length === 0 ? <Typography color="text.secondary">条件に合う繁殖記録はありません。</Typography> : isMobile ? (
          ...
          <Box sx={{ overflowX: 'auto' }}><Table size="small" sx={{ minWidth: 1120 }}><TableHead><TableRow>
            ...
            {filteredItems.map((item) => <TableRow key={item.id} hover>
              ...
              <TableCell>{item.breedingMethod || '未選択'}</TableCell><TableCell>{displayDate(performedDate(item))}</TableCell>
              <TableCell><Typography variant="body2">父牛：{breedingSire(item)}</Typography><Typography variant="caption">担当：{breedingActor(item)}</Typography>{item.breedingMethod === '受精卵移植' && <Typography variant="caption" display="block">購入先：{breedingPartner(item)}</Typography>}</TableCell>
              <TableCell><Chip size="small" label={currentStage(item)} color={stageColor(currentStage(item)) as any} /></TableCell>
              <TableCell><Typography variant="body2">{nextAction(item).label}</Typography><Typography variant="caption">{displayDate(nextAction(item).date)}</Typography></TableCell>
