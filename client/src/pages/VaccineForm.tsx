...
export function VaccineForm({ mode }: Props) {
  ...
  const setValue = (key: keyof VaccineInput, value: string) => setForm((prev) => ({ ...prev, [key]: value }));

  const handleSubmit = async () => {
    if (!form.targetType || !form.targetNumber || !form.targetName || !form.vaccineName) {
      alert('対象区分、対象番号、対象名、ワクチン名は必須です');
      return;
    }
    try {
  ...
  return (
    <Stack spacing={2}>
      ...
      <Card><CardContent><Stack spacing={2}>
        ...
        <Typography color="text.secondary">判定：{label}{form.nextDueDate ? ` / 次回予定まであと${daysUntil(form.nextDueDate)}日` : ''}</Typography>
