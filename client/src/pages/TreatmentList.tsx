...
export function TreatmentList() {
  ...
  const filterActive = Boolean(keyword.trim() || progress !== 'すべて' || recordType !== 'すべて');
        ...
        <Stack spacing={1} sx={{ display: { xs: 'flex', md: 'none' } }}>
          {filteredItems.map((item) => {
            ...
            return <Card key={item.id}><CardContent><Stack spacing={1}>
              ...
              <Typography><b>治療日：</b>{item.treatmentDate || '-'}</Typography><Typography><b>薬剤：</b>{item.medicine || '-'}</Typography>
      ...
      <CollapsibleSearchPanel active={filterActive}>
        <Stack spacing={1}>
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
            <TextField label="記録区分" select value={recordType} onChange={(e) => setRecordType(e.target.value)} size="small" sx={{ minWidth: 160 }}><MenuItem value="すべて">すべて</MenuItem><MenuItem value="治療">治療</MenuItem><MenuItem value="予防">予防</MenuItem><MenuItem value="去勢">去勢</MenuItem><MenuItem value="削蹄">削蹄</MenuItem><MenuItem value="その他の処置">その他の処置</MenuItem></TextField>
