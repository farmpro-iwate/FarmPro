    ...
    <Stack spacing={1.5}>
        ...
        {loading ? <Typography>読み込み中...</Typography> : <>
          <Stack spacing={1} sx={{ display: { xs: 'flex', md: 'none' } }}>
            {filteredItems.map((item) => {
              ...
              return <Card key={item.id} variant="outlined"><CardContent><Stack spacing={1}>
          ...
          <Box sx={{ display: { xs: 'none', md: 'block' }, overflowX: 'auto' }}>
            <Table size="small"><TableHead><TableRow><TableCell>予定区分</TableCell><TableCell>予定内容</TableCell><TableCell>対象</TableCell><TableCell>予定日</TableCell><TableCell>判定</TableCell><TableCell align="right">操作</TableCell></TableRow></TableHead>
      ...
      <CollapsibleSearchPanel active={hasFilters}>
        <Stack spacing={1}>
          ...
          <Button variant="outlined" onClick={clearSearch} size="small" disabled={!hasFilters}>検索条件をクリア</Button>
        </Stack>
