    ...
    <Stack spacing={2}>
      ...
      <Card className="print-card"><CardContent><Stack spacing={2}>
        ...
        <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
          <Card variant="outlined" sx={{ flex: 1 }}><CardContent><Stack spacing={0.75}><Typography fontWeight={900}>今の状態</Typography><Typography fontWeight={800}>現在産次：{parityCount > 0 ? `${parityCount}産` : '未経産'}</Typography><Typography fontWeight={800}>空胎日数：{openDays ? `${openDays.days}日（${openDays.status}）` : '算出不可'}</Typography><Typography color="text.secondary">直近分娩日：{openDays?.latestCalvingDate || '-'}</Typography></Stack></CardContent></Card>
          ...
          <Card variant="outlined" sx={{ flex: 1 }}><CardContent><Stack spacing={0.75}><Typography fontWeight={900}>子牛とのつながり</Typography>{latestCalf ? <><Typography fontWeight={800}>直近の子牛：{value(latestCalf.earTag || latestCalf.name || latestCalf.calfNumber)}</Typography><Typography color="text.secondary">生年月日：{value(dateOnly(latestCalf.birthDate || latestCalf.birthday))}</Typography><Typography color="text.secondary">性別：{formatSex(latestCalf.sex)}</Typography><Button component={RouterLink} to={`/calves/${latestCalf.id}`} variant="outlined" size="small" className="no-print" sx={{ alignSelf: 'flex-start' }}>子牛を見る</Button></> : <Typography color="text.secondary">この個体に連動する子牛はまだありません。</Typography>}</Stack></CardContent></Card>
        </Stack>
