...
export function CattleDetail() {
  ...
  const aiRecordQuery = new URLSearchParams({ mode: 'field', targetType: 'cattle', targetNumber: cattle.earTag || '', targetName: cattle.name || '', cattleId: cattle.id || '', returnTo: `/cattle/${cattle.id}` }).toString();
  ...
  return (
    <Stack spacing={1.5}>
      <Stack direction="row" spacing={1} className="no-print">
        <Button component={RouterLink} to={isSold ? '/cattle/sold' : '/cattle'} variant="outlined">{isSold ? '販売済み牛一覧へ戻る' : '個体一覧へ戻る'}</Button>
      ...
      <Card className="print-card"><CardContent sx={{ p: { xs: 1.5, sm: 2 }, '&:last-child': { pb: { xs: 1.5, sm: 2 } } }}><Stack spacing={1.25}>
              ...
              <Stack spacing={0.8}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 0.75, sm: 4 }} useFlexGap flexWrap="wrap">
                  <Stack spacing={0.05} sx={{ minWidth: 130 }}>
                    ...
                    <Typography fontWeight={800}>{dateOnly(soldSale?.saleDate) || '-'}</Typography>
                  </Stack>
                  <Stack spacing={0.05} sx={{ minWidth: 150 }}>
                    <Typography variant="body2" color="text.secondary">販売額</Typography>
                    <Typography variant="h6" fontWeight={900}>{soldSalePrice !== null ? `${Math.round(soldSalePrice).toLocaleString('ja-JP')}円` : '-'}</Typography>
                  </Stack>
                  <Stack spacing={0.05} sx={{ minWidth: 150 }}>
                    <Typography variant="body2" color="text.secondary">販売時生産費</Typography>
                    <Typography variant="h6" fontWeight={900}>{soldProductionCost !== null ? `${Math.round(soldProductionCost).toLocaleString('ja-JP')}円` : '-'}</Typography>
                  </Stack>
                  <Stack spacing={0.05} sx={{ minWidth: 150 }}>
                    <Typography variant="body2" color="text.secondary">利益</Typography>
                    <Typography variant="h6" fontWeight={900}>{soldProfit !== null ? `${Math.round(soldProfit).toLocaleString('ja-JP')}円` : '-'}</Typography>
                  </Stack>
                </Stack>
        ...
        {timeline.length === 0 ? <Alert severity="info">この牛の活動記録はまだありません。</Alert> : <Stack spacing={0.6}>{timeline.map((item) => <Card key={item.id} variant="outlined"><CardActionArea component={RouterLink} to={item.to}><CardContent sx={{ py: 0.75, px: 1.25, '&:last-child': { pb: 0.75 } }}><Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.75} alignItems={{ sm: 'center' }}><Typography fontWeight={900} sx={{ minWidth: 105 }}>{item.date}</Typography><Chip size="small" label={item.category} /><Stack spacing={0.1} sx={{ flexGrow: 1 }}><Typography fontWeight={800}>{item.title}</Typography><Typography color="text.secondary">{item.detail}</Typography></Stack><Typography color="primary" fontWeight={800}>記録を確認 →</Typography></Stack></CardContent></CardActionArea></Card>)}</Stack>}
        <Divider sx={{ my: 0.25 }} />
        <Typography variant="h6" fontWeight={800}>基本情報</Typography>
        <Table size="small" sx={{ display: { xs: 'none', sm: 'table' }, '& td': compactCellSx }}>
          <TableBody>
            <TableRow><TableCell sx={{ fontWeight: 700 }}>耳標番号</TableCell><TableCell>{value(cattle.earTag)}</TableCell><TableCell sx={{ fontWeight: 700 }}>個体識別番号</TableCell><TableCell>{value(cattle.identificationNumber)}</TableCell></TableRow>
            <TableRow><TableCell sx={{ fontWeight: 700 }}>名号</TableCell><TableCell>{value(cattle.name)}</TableCell><TableCell sx={{ fontWeight: 700 }}>生年月日</TableCell><TableCell>{value(cattle.birthday)}</TableCell></TableRow>
            <TableRow><TableCell sx={{ fontWeight: 700 }}>産次</TableCell><TableCell>{parityCount > 0 ? `${parityCount}産` : '未経産'}</TableCell><TableCell sx={{ fontWeight: 700 }}>父牛</TableCell><TableCell>{value(cattle.sire)}</TableCell></TableRow>
            <TableRow><TableCell sx={{ fontWeight: 700 }}>母牛</TableCell><TableCell>{value(cattle.dam)}</TableCell><TableCell sx={{ fontWeight: 700 }}>備考</TableCell><TableCell>{value(cattle.note)}</TableCell></TableRow>
            {cattle.sourceCalfId && <TableRow><TableCell sx={{ fontWeight: 700 }}>移行元</TableCell><TableCell colSpan={3}><Button component={RouterLink} to={`/calves/${cattle.sourceCalfId}`} size="small" variant="outlined" className="no-print">子牛の元記録を見る</Button></TableCell></TableRow>}
          </TableBody>
        </Table>
        <Table size="small" sx={{ display: { xs: 'table', sm: 'none' }, '& td': compactCellSx }}><TableBody><TableRow><TableCell>耳標番号</TableCell><TableCell>{value(cattle.earTag)}</TableCell></TableRow><TableRow><TableCell>個体識別番号</TableCell><TableCell>{value(cattle.identificationNumber)}</TableCell></TableRow><TableRow><TableCell>名号</TableCell><TableCell>{value(cattle.name)}</TableCell></TableRow><TableRow><TableCell>生年月日</TableCell><TableCell>{value(cattle.birthday)}</TableCell></TableRow><TableRow><TableCell>産次</TableCell><TableCell>{parityCount > 0 ? `${parityCount}産` : '未経産'}</TableCell></TableRow><TableRow><TableCell>父牛</TableCell><TableCell>{value(cattle.sire)}</TableCell></TableRow><TableRow><TableCell>母牛</TableCell><TableCell>{value(cattle.dam)}</TableCell></TableRow>{cattle.sourceCalfId && <TableRow><TableCell>移行元</TableCell><TableCell><Button component={RouterLink} to={`/calves/${cattle.sourceCalfId}`} size="small" variant="outlined" className="no-print">子牛の元記録を見る</Button></TableCell></TableRow>}<TableRow><TableCell>備考</TableCell><TableCell>{value(cattle.note)}</TableCell></TableRow></TableBody></Table>
        <Divider sx={{ my: 0.25 }} />
        <Typography variant="h6" fontWeight={800}>繁殖成績</Typography>
        <Stack direction="row" spacing={0.6} useFlexGap flexWrap="wrap">
          <Chip size="small" label={`産次：${parityCount > 0 ? `${parityCount}産` : '未経産'}`} />
          <Chip size="small" label={`累計種付回数：${serviceCount}回`} />
          <Chip size="small" label={`平均空胎日数：${breedingPerformance.averageOpenDays !== null ? `${breedingPerformance.averageOpenDays}日` : '算出不可'}`} />
          <Chip size="small" label={`平均妊娠期間：${breedingPerformance.averageGestationDays !== null ? `${breedingPerformance.averageGestationDays}日` : '算出不可'}`} />
          <Chip size="small" label={`平均分娩間隔：${breedingPerformance.averageCalvingInterval !== null ? `${breedingPerformance.averageCalvingInterval}日` : '算出不可'}`} />
        </Stack>
