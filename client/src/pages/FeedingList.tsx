...
import { deleteFeeding, FeedingRecord, feedingPurposeOptions, feedingUnitOptions, getFeedingsList } from '../services/feedingsApi';
...
function amountWithUnit(amount: string, unit: string) { if (!amount) return '-'; const n = Number(amount); if (Number.isNaN(n)) return `${amount}${unit || ''}`; return `${n.toLocaleString('ja-JP')}${unit || ''}`; }
...
function downloadFeedingsCsv(rows: FeedingRecord[]) {
  ...
  const csv = '\ufeff' + [headers.map(csvEscape).join(','), ...body.map((line) => line.map((item) => csvEscape(rawValue(item))).join(','))].join('\r\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a'); link.href = url; link.download = `farmpro_feedings_${todayText()}.csv`; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
}
...
export function FeedingList() {
  ...
  async function loadFeedings() { setLoading(true); setError(''); try { setRows(await getFeedingsList()); } catch (err) { setError(err instanceof Error ? err.message : '飼料給与記録を取得できませんでした。'); } finally { setLoading(false); } }
  useEffect(() => { loadFeedings(); }, []);

  async function handleDelete(row: FeedingRecord) {
    if (!window.confirm(`この飼料給与記録を削除しますか？\n\n給与日：${row.feedingDate || '-'}\n対象：${row.target || '-'}\n飼料名：${row.feedName || '-'}`)) return;
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      if (q && ![row.feedingDate,row.target,row.feedName,row.amount,row.unit,row.unitPrice,row.totalPrice,row.purpose,row.memo].join(' ').toLowerCase().includes(q)) return false;
  ...
  return (
    <Stack spacing={2}>
      ...
      <Stack spacing={0.5} className="print-only">
        ...
        <Typography>表示件数：{filteredRows.length}件 / 給与量合計：{totalAmount.toLocaleString('ja-JP')} / 金額合計：{totalPrice.toLocaleString('ja-JP')}円</Typography>
      </Stack>

      <Alert severity="info" className="no-print">飼料給与記録の一覧です。必要に応じて下部の検索・絞り込みを使えます。表示中のデータだけCSV出力・印刷できます。</Alert>
      ...
      <Card className="no-print"><CardContent><Stack spacing={1}>
        ...
        <Typography>全件数：{rows.length}件</Typography><Typography>表示件数：{filteredRows.length}件</Typography>
        <Typography>表示中の給与量合計：{totalAmount.toLocaleString('ja-JP')}</Typography><Typography>表示中の金額合計：{totalPrice.toLocaleString('ja-JP')}円</Typography>
      ...
      {!loading && !error && filteredRows.length > 0 && <>
        <Stack spacing={1.5} sx={{ display: { xs: 'flex', md: 'none' } }} className="no-print">
          {filteredRows.map((row) => <Card key={row.id}><CardContent><Stack spacing={1.25}>
            ...
            <Divider />
            <Typography><b>対象：</b>{value(row.target)}</Typography><Typography><b>給与量：</b>{amountWithUnit(row.amount, row.unit)}</Typography><Typography><b>単価：</b>{yen(row.unitPrice)}</Typography><Typography><b>金額：</b>{yen(row.totalPrice)}</Typography>
