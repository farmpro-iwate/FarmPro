...
function hasTaxExcludedPurchase(rows: FeedInventoryRecord[], feedName: string) {
  return rows.some((row) =>
    row.feedName.trim() === feedName.trim() &&
    row.transactionType === '入庫' &&
    Number(row.taxExcludedPrice || 0) > 0,
  );
}
...
function recalculateCosting(costing: FeedCostingSnapshot, averageUnitCost: number) {
  ...
  const allocations = costing.allocations.map((item) => ({
    ...item,
    allocatedCost: item.allocatedQuantity * averageUnitCost,
  }));

  const actualIntake = costing.actualIntake
    ? {
...
export async function reconcileTaxAdjustedOutboundCosts(rows: FeedInventoryRecord[]) {
  ...
  for (const row of ordered) {
    ...
    if (!hasTaxExcludedPurchase(ordered, row.feedName)) {
      working.push(row);
      continue;
    }
