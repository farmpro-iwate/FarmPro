...
type StoredCalvingRecord = CalvingRecord & {
  id: string;
};
...
function normalizeRecord(
  ...
  return {
    ...
    memo: input.memo || '',
    registeredToCalfLedger: Boolean(
      input.registeredToCalfLedger ?? existing?.registeredToCalfLedger,
    ),
...
function withComputedFields(record: StoredCalvingRecord): CalvingRecord {
  return {
    ...
    daysFromExpected: daysDifference(
...
function isDuplicateCalf(calves: StoredCalfRecord[], record: StoredCalvingRecord) {
  ...
  return calves.some((calf) => {
    const sameName =
      Boolean(calfName) &&
      [calf.name, calf.earTag].some(
        (value) => String(value || '').trim() === calfName,
      );
    ...
    const sameBirthMother =
      ...
      ((Boolean(motherCowId) &&
        String(calf.motherCowId || '').trim() === motherCowId) ||
        (Boolean(motherCowName) &&
          String(calf.motherCowName || '').trim() === motherCowName));
...
export async function createCalving(record: CalvingRecord) {
  const saved = await saveRecord('calvings', normalizeRecord(record));
...
export async function updateCalving(id: string, record: CalvingRecord) {
  ...
  const saved = await saveRecord(
    'calvings',
    normalizeRecord({ ...record, id }, existing),
  );
  return withComputedFields(saved);
}
...
export async function registerCalvingToCalfLedger(
  id: string,
): Promise<RegisterCalfResponse> {
  const record = await getRecordById<StoredCalvingRecord>('calvings', id);
  ...
  if (normalizeCalvingResult(record.calvingResult) === '死産') {
    throw new Error('死産の記録は子牛台帳へ登録しません。');
  }

  if (!record.calfName) {
    throw new Error('子牛耳標番号がないため、子牛台帳へ登録できません。');
  }

  if (!record.actualCalvingDate) {
    throw new Error('実分娩日がないため、子牛台帳へ登録できません。');
  }
  ...
  if (isDuplicateCalf(calves, record)) {
    throw new Error(
      '同じ子牛耳標番号、または同じ母牛・生年月日の子牛がすでに子牛台帳にある可能性があります。重複を確認してください。',
    );
  }
  ...
  const memoLines = [
    ...
    record.calvingResult
      ? `分娩結果: ${normalizeCalvingResult(record.calvingResult)}`
      : '',
  ...
  const calf: StoredCalfRecord = {
    ...
    birthWeightKg:
      record.birthWeightKg === undefined ? '' : record.birthWeightKg,
  ...
  return {
    ok: true,
    calf,
    calving: withComputedFields(updatedCalving),
  };
}
