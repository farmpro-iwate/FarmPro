...
function cattleSaleDate(cattle: Cattle, sales: SaleRecord[]) {
  const dates = sales
    .filter((sale) => saleMatchesCattle(sale, cattle))
...
function cattleStay(
  ...
  const soldAt = cattleSaleDate(cattle, sales);
  const endDate = soldAt ? minDate(today, soldAt) : today;
...
function daysInPeriod(stay: CattleStay, period: ExpensePeriodKey) {
...
export async function getCattleFarmExpenseAllocation(
  cattleId: string,
): Promise<number> {
  ...
  if ((settings.farmExpenseAllocationTarget || 'all') !== 'cattle') {
    return 0;
  }
  ...
  const [cattleList, expenses, sales] = await Promise.all([
    getAllRecords<Cattle>('cattle'),
    getExpensesList(),
    getSalesList(),
  ]);

  const targetCattle = cattleList.find(
    (cattle) => String(cattle.id) === String(cattleId),
  );

  if (!targetCattle || !isBreedingCattle(targetCattle)) {
    return 0;
  }

  const today = dateOnly(new Date());

  const stays = cattleList
    .map((cattle) => cattleStay(cattle, sales, today))
    .filter((stay): stay is CattleStay => Boolean(stay));

  const targetStay = stays.find(
    (stay) => String(stay.cattle.id) === String(cattleId),
  );
