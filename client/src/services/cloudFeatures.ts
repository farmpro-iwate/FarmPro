...
export async function saveToCloud(): Promise<{ savedAt: string; revision: number; exportedAt: string; appVersion: string; }> {
...
export async function getCloudRestorePreview(): Promise<CloudRestorePreview | null> {
  ...
  const recordCount = Object.values(backup.stores).reduce((total, records) => total + records.length, 0);
  return { savedAt: stored.savedAt, revision: stored.revision, exportedAt: backup.exportedAt, appVersion: backup.appVersion, recordCount, backup };
}
...
export async function restoreLatestFromCloud(backup: FarmProBackup): Promise<void> {
  ...
  if (!current) throw new CloudSnapshotConflictError('クラウドデータが変更されています。もう一度内容を確認してください。');
