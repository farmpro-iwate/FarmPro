...
function webhookSecrets() {
  return {
    live: process.env.STRIPE_WEBHOOK_SECRET?.trim() || '',
    test: process.env.STRIPE_WEBHOOK_TEST_SECRET?.trim() || '',
  };
}
...
async function handleCheckoutCompleted(object: Record<string, unknown>, mode: WebhookMode) {
  ...
  if (!userId && mode === 'live') throw new Error('INVALID_CLIENT_REFERENCE_ID');
...
async function processStripeEvent(event: StripeEvent, mode: WebhookMode) {
  const events = await processedEventIds();
  ...
  if (event.type === 'checkout.session.completed') {
    await handleCheckoutCompleted(object, mode);
...
export async function stripeWebhookHandler(req: Request, res: Response) {
  ...
  if (signature && rawBody.length > 0) {
    if (secrets.live && verifyStripeSignature(rawBody, signature, secrets.live)) mode = 'live';
    else if (secrets.test && verifyStripeSignature(rawBody, signature, secrets.test)) mode = 'test';
  }
  ...
  try {
    await processStripeEvent(event, mode);
