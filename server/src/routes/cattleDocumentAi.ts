...
const candidateSchema = {
  ...
  required: ['identificationNumber','sourceReferenceNumber','registrationNumber','name','birthday','sire','dam','maternalSire','maternalGrandSire','offspring','notes'],
} as const;

type RequestBody = { fileName?: string; mimeType?: string; base64?: string };
...
cattleDocumentAiRouter.post('/', async (req, res) => {
  ...
  try {
    ...
    const response = await client.responses.create({
      ...
      text: { format: { type: 'json_schema', name: 'farmpro_cattle_document_candidate', strict: true, schema: candidateSchema } },
    });
    ...
    res.status(502).json({ message: caught instanceof Error ? caught.message : 'AI画像解析に失敗しました。' });
  }
});
