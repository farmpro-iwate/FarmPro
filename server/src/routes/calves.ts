...
calvesRouter.post('/:id/promote', async (req, res) => {
  ...
  if (calf.sex !== '雌') {
    res.status(400).json({ message: '個体カルテへ移行できるのは雌の子牛です' });
    return;
  }
  if (calf.managementStatus === '牛台帳へ移行済み' && calf.promotedCattleId) {
    res.status(400).json({ message: 'この子牛はすでに個体カルテへ移行済みです' });
    return;
  }
  try {
    const cattle = await createCattle({
      ...
      note: [calf.note, '子牛管理から個体カルテへ移行'].filter(Boolean).join(' / '),
    });
